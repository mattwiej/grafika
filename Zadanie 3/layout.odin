...
PropertyInputCMYK :: proc(label: string, value: ^$T, id: u32, state: ^State_models) {
...
modesOverlay :: proc(state: ^State_models) {
	if clay.UI(clay.ID("ToolsWindow"))(
	{
		...
		if clay.UI(clay.ID("RGB"))(
		{
			...
			PropertyInput("r", &state.rgb.r, 420, state)
			PropertyInput("g", &state.rgb.g, 421, state)
			PropertyInput("b", &state.rgb.b, 422, state)
		}

		if clay.UI(clay.ID("CMYK"))(
		{
			layout = {
				sizing = {width = clay.SizingGrow({}), height = clay.SizingFit({})},
				layoutDirection = .TopToBottom,
			...
			PropertyInputCMYK(" c", &state.cmyk.C, 520, state)
			PropertyInputCMYK("m", &state.cmyk.M, 521, state)
			PropertyInputCMYK(" y", &state.cmyk.Y, 522, state)
			PropertyInputCMYK(" k", &state.cmyk.K, 523, state)
		}
		...
		if clay.UI(clay.ID("ColorPreview", 405))(
		{
			...
			backgroundColor = {state.rgb.r, state.rgb.g, state.rgb.b, 255},
			cornerRadius = clay.CornerRadiusAll(5),
